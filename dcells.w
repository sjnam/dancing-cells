...
@ 엔진을 셋 두고 한 지붕 아래 산다. 앞의 둘은 묻는 것이 갈린다. ``꼭 한 번''이
느슨해지는 길이 자연스럽게 둘이고, 그 둘이 저마다 제 기계를 가질 만해서다.
셋째는 앞의 하나와 같은 것을 묻되 마디마다 들이는 품이 다르다.
...
셋째 |XCCDC|는 |XCC|와 똑같은 물음에 답하되 훨씬 멀리 내다본다. 어떤 옵션을 {\it
쓰기만 해도\/} 다른 어딘가의 주 아이템이 덮일 길을 잃는다면 그 옵션을 그 자리에서
내버리고, 그 치움이 잦아들 때까지 물결치게 둔다. {\it 도메인 일관성\/}이라 부르는
성질이다. 마디는 비싸지고 그 수는 훨씬 줄어드는데, 그 거래가 값을 하는지는 오롯이
문제에 달렸다. 그래서 이 엔진도 |XCC|를 대신하지 않고 그 곁에 선다.

@ Go다운 자유를 하나 누렸는데, 그것이 세 엔진에 두루 흐른다. 크누스의 풀이기는
해를 하나 찾을 때마다 표준 오류 스트림에 찍고 내처 나아가지만, 이쪽은 해마다
채널로 돌려준다. 부르는 쪽은 |dcells.NewXCC()|나 |dcells.NewMCC()|나
|dcells.NewXCCDC()|로 풀이기를 짓고, 입력에 대고 |Dance(reader)|를 부른 뒤
|res.Solutions|를 훑으면 된다. 닿는 값마다 |[]Option|이고 |Option| 하나는 아이템
이름의 |[]string|인데, 색이 붙은 부 아이템은 |name:color| 꼴로 나온다. 탐색은 제
고루틴에서 돌며 보낼 때마다 막히므로, 해를 훑는 쪽이 탐색의 걸음을 쥔다. 듣기를 그만두면 탐색도 멈춘다. 이
...
그 공용 바탕이다. 공개 API의 모양, |XCC|와 |MCC|가 함께 춤추는 노드 배열,
그리고 {\tt DLX} 텍스트를 읽어 들이는 손이 여기 있다. 엔진은 이웃에 산다---{\tt
SSXCC}는 \.{ssxcc.w}에, {\tt SSMCC}는 \.{ssmcc.w}에, {\tt XCCDC}는
\.{xccdc.w}에 있고, 크누스의 {\tt DLX1}, {\tt DLX2}, {\tt DLX3}이 그랬듯 어느
쪽이든 다른 쪽 없이 처음부터 끝까지 읽힌다. 아래가 그 공용 바탕의 뼈대다.
@c
...
@ 엔진들이 나눠 쓰는 파수꾼이 셋이다. 값 |infSize|는 실제 옵션 수보다 언제나
...
@ 값을 따져 푸는 두 엔진 |XCC|와 |MCC|는 옵션을 모두 {\it 순값\/}---값에서
...
@** DLX 입력 읽기.
세 엔진은 같은 형식을 먹는다. 크누스의 풀이기들이 오래 써 온 {\tt DLX}
...
춤이 기다리는 희소 집합을 깔아 두는 {\it 마무리\/}가 따른다. 걸음이 갈리는 자리는
몇 군데뿐이다. 엔진 |MCC|는 다중도를 읽고, |XCCDC|는 옵션마다 주 아이템이 앞에
서도록 노드를 옮겨 놓는다. Go의 타입 체계로는 그 코드를 함께 쓰려다 더
번거로워지므로 엔진마다 제 사본을 두었고, 뒤의 두 글은 달라지는 대목만 짚는다.
@<입력 훑개@>=
@<읽기 실패@>
@<한 줄 읽기@>
@<낱말 훑기@>
