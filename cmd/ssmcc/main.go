// Command ssmcc is an exact-cover solver with item multiplicities and colors,
// using sparse-set "dancing cells" data structures and binary branching.
//
// It is a Go port of Donald E. Knuth's CWEB program SSMCC
// (https://www-cs-faculty.stanford.edu/~knuth/programs/ssmcc.w), which extends
// SSXCC with the multiplicity (bound/slack) machinery of DLX3. The inert
// "weight" scaffolding of the original (never read by the search) is omitted.
//
//	ssmcc [flags] [file.dlx]
//	ssmcc -m 1 < file.dlx
package main
...
func report() {
	...
	if baditem != 0 {
		if vbose&showChoices != 0 {
			k := int(item[baditem-1])
			fmt.Fprint(os.Stderr, "Item")
			printItemName(k, os.Stderr)
			fmt.Fprintf(os.Stderr, " has fewer than %d options!\n", bound(k)-slack(k))
		}
	} else {
		solve()
	}
	...
	if vbose&showMaxDeg != 0 {
		fmt.Fprintf(os.Stderr, "The maximum best_itm size was %d.\n", maxdeg)
	}
	if vbose&showBasics != 0 {
		plural := "s"
		if count == 1 {
