...
```sh
go run ./cmd/ssxcc -m 1 cmd/ssxcc/examples/exactcover.dlx   # 모든 해를 출력
go run ./cmd/ssxcc < cmd/ssxcc/examples/exactcover.dlx      # 표준입력으로도 가능
python3 cmd/ssxcc/examples/queens.py 8 | go run ./cmd/ssxcc -m 1
go build -o ssxcc ./cmd/ssxcc                               # 바이너리로 빌드하려면
```
...
다중도(multiplicity)를 더한 자매 프로그램은 [`../ssmcc`](../ssmcc/README.md)를
보세요.
